...
function ReviewRequest(userid,caregiverid){
	
	localStorage.setItem("UserIDforReview",userid);
	localStorage.setItem("Caregiveridforreview",caregiverid);
	
    window.location.href="writecaregiverreview.html";
}
function populateUserCurrentRequests(userId)
{
	console.log("Populating populateUserCurrentRequests Profile");
	 var postString = "category=1&UserId="+userId+"&approveStatus=0";
	 $.ajax({
				url:'http://localhost/tend_services/userCareGiverEngagements.php',
				type:"POST",
				data:postString,
...
{	
	...
	htmlString=htmlString+ "<a href=\"userpayCareGiver.html\" class=\"cancelJob\"><img src=\"images/pay-caregiver.png\" class=\"paycaregiver-icon\"></a>";
	htmlString=htmlString+ "<a href=\"#\" class=\"cancelJob\"><img src=\"images/star-new.png\" class=\"review-icon\" onClick=\"ReviewRequest("+viewData.requester_ID+","+viewData.caregiver_ID+")\"></a>";
	...
	htmlString=htmlString+ "</li>";
	$("#user_curr_req").append(htmlString);
}
